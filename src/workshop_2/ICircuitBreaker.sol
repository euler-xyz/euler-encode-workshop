...
interface ICircuitBreaker {
    ...
    /// For the native asset (ETH on mainnet), MUST be address 0x0000000000000000000000000000000000000001 equivalent to
    /// address(1).
    /// @param metricThreshold The threshold metric which defines when a rate limit is triggered
    /// @param minAmountToLimit The minimum amount of nominal asset liquidity at which point rate limits can be
    /// triggered
    ...
    /// @dev Each asset that will be rate limited MUST be registered using this function, including the native asset
    /// (ETH on mainnet).
    /// If an asset is not registered, it will not be subject to rate limiting or circuit breaking and unlimited
    /// immediate withdrawals MUST be allowed.
    ...
    /// To set rate limit parameters For the native asset, MUST be address 0x0000000000000000000000000000000000000001
    /// equivalent to address(1).
    /// @param _metricThreshold The threshold metric which defines when a rate limit is triggered.
    /// This is intentionally left open to allow for various implementations, including percentage-based (see reference
    /// implementation), nominal, and more.
    /// MUST be greater than 0.
    /// @param _minAmountToLimit The minimum amount of nominal asset liquidity at which point rate limits can be
    /// triggered.
    /// This limits potential false positives triggered either by minor assets with low liquidity or by low liquidity
    /// during early stages of protocol launch.
    ...
    /// To update the rate limit parameters For the native asset (ETH on mainnet), MUST be address
    /// 0x0000000000000000000000000000000000000001 equivalent to address(1).
    /// @param _metricThreshold The threshold metric which defines when a rate limit is triggered.
    /// This is left open to allow for various implementations, including percentage-based (see reference
    /// implementation), nominal, and more.
    /// MUST be greater than 0.
    /// @param _minAmountToLimit The minimum amount of nominal asset liquidity at which point rate limits can be
    /// triggered.
    /// This limits potential false positives caused both by minor assets with low liquidity and by low liquidity during
    /// early stages of protocol launch.
    /// Below this amount, withdrawals of this asset MUST NOT trigger a rate limit.
    ...
    /// @dev This method MUST be called from all protected contract methods where an EIP-20 token is transferred in from
    /// a user.
    ...
    /// @notice Record EIP-20 token outflow from a protected contract and transfer tokens to recipient if rate limit is
    /// not triggered
    /// @dev This method MUST be called from all protected contract methods where an EIP-20 token is transferred out to
    /// a user.
    /// Before calling this method, the protected contract MUST transfer the EIP-20 tokens to the circuit breaker
    /// contract.
    ...
    /// If a rate limit is not triggered or the circuit breaker is in grace period, this method MUST NOT revert and MUST
    /// transfer the tokens to the recipient.
    /// If a rate limit is triggered and the circuit breaker is not in grace period and `_revertOnRateLimit` is TRUE,
    /// this method MUST revert.
    /// If a rate limit is triggered and the circuit breaker is not in grace period and `_revertOnRateLimit` is FALSE
    /// and caller is a protected contract, this method MUST NOT revert.
    /// If a rate limit is triggered and the circuit breaker is not in grace period, this method MUST record the locked
    /// funds in the internal accounting of the circuit breaker implementation.
    /// @param _token MUST be an EIP-20 token contract
    ...
    /// @param _revertOnRateLimit MUST be TRUE to revert if a rate limit is triggered or FALSE to return without
    /// reverting if a rate limit is triggered (delayed settlement)
    ...
    /// @dev This method MUST be called from all protected contract methods where native asset is transferred in from a
    /// user.
    /// MUST revert if caller is not a protected contract.
    /// MUST revert if circuit breaker is not operational.
    ...
    /// @notice Record native asset (ETH on mainnet) outflow from a protected contract and transfer native asset to
    /// recipient if rate limit is not triggered
    /// @dev This method MUST be called from all protected contract methods where native asset is transferred out to a
    /// user.
    /// When calling this method, the protected contract MUST send the native asset to the circuit breaker contract in
    /// the same call.
    /// For an example, see ProtectedContract.sol in the reference implementation.
    /// MUST revert if caller is not a protected contract.
    /// MUST revert if circuit breaker is not operational.
    /// If native asset is not registered, this method MUST NOT revert and MUST transfer the native asset to the
    /// recipient.
    /// If a rate limit is not triggered or the circuit breaker is in grace period, this method MUST NOT revert and MUST
    /// transfer the native asset to the recipient.
    /// If a rate limit is triggered and the circuit breaker is not in grace period and `_revertOnRateLimit` is TRUE,
    /// this method MUST revert.
    /// If a rate limit is triggered and the circuit breaker is not in grace period and `_revertOnRateLimit` is FALSE
    /// and caller is a protected contract, this method MUST NOT revert.
    /// If a rate limit is triggered and the circuit breaker is not in grace period, this method MUST record the locked
    /// funds in the internal accounting of the circuit breaker implementation.
    /// @param _recipient MUST be the address of the recipient of the transferred native asset from the protected
    /// contract
    /// @param _revertOnRateLimit MUST be TRUE to revert if a rate limit is triggered or FALSE to return without
    /// reverting if a rate limit is triggered (delayed settlement)
    ...
    /// @dev When a asset is transferred out during a rate limit period, the settlement may be delayed and the asset
    /// custodied in the circuit breaker.
    /// This method allows users to claim funds that were delayed in settlement after the rate limit is resolved or a
    /// grace period is activated.
    ...
    /// MUST update internal accounting of circuit breaker implementation to reflect withdrawn balance on successful
    /// call.
    ...
    /// The admin has authority to: withdraw locked funds, set grace periods, register asset parameters, update asset
    /// parameters,
    ...
    /// MUST update the circuit breaker admin to the new admin in the stored state of the implementation on successful
    /// call.
    /// @param _newAdmin MUST be the address of the new admin
    function setAdmin(address _newAdmin) external;

    /// @notice Override a rate limit
    /// @dev This method MAY be called when the protocol admin (typically governance) is certain that a rate limit is
    /// the result of a false positive.
    /// MUST revert if caller is not the current admin.
    /// MUST allow the grace period to extend for the full withdrawal period to not trigger the rate limit again if the
    /// rate limit is removed just before the withdrawal period ends.
    ...
    /// For the native asset (ETH on mainnet), MUST be address 0x0000000000000000000000000000000000000001 equivalent to
    /// address(1).
    /// @param _recoveryRecipient The address of the recipient to receive recovered funds. Often this will be a multisig
    /// wallet.
    ...
    /// To check if triggered For the native asset (ETH on mainnet), this MUST be address
    /// 0x0000000000000000000000000000000000000001 equivalent to address(1).
    /// @return isRateLimitTriggered MUST return TRUE if a rate limit is currently triggered for given asset, FALSE if
    /// not
